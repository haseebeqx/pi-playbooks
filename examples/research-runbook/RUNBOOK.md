...
2. Call `runbook_checkpoint` with stage `plan`, the plan artifact path, and an approval gate whose ID is `approve-plan`.
...
4. Call `runbook_checkpoint` with stage `evidence` and a concise summary.

## 3. Report

1. Write `results/report.md` with findings, sources, limitations, and falsifiable next steps.
2. Call `runbook_checkpoint` with stage `report` and the report artifact path.
3. Call `runbook_finish` with outcome `success`. The runtime will reject completion if the report is absent or empty, then keep the run open for user questions and requested changes until the user runs `/runbook close`.
